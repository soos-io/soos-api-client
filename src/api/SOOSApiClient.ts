        ...
        if (axios.isAxiosError(error)) {
          ...
          if (config && response) {
            switch (response.status) {
              ...
              case 403:
                throw new Error(
                  `Please Verify your API Key and Client ID and ensure the align with an appropriate Role within SOOS to run scans. (Forbidden - ${apiClientName} - ${config.method} ${config.url})`,
                );
              case 429:
                throw new Error(
